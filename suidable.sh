...
function helpPanel(){
  ...
  echo -e "\t${purpleColor}[!]${endColor} ${turquoiseColor}IMPORTANT ACTIONS: ${endColor}${blueColor}Make sure to always update the necessary files using parameter ${endColor}${yellowColor}-u ${endColor}${blueColor}before every use to make sure the data collected on the system is up to date and matches the latest findings stored on GTFOBINS${endColor}\n"
